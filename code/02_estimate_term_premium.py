...
################################################################################
r_g_master = pd.read_csv(raw_data / 'r_g_master.csv', parse_dates=['date'])
...
for col in ['r', 'g']:
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(exploratory / f'{col}_comparison.png', dpi=300)
    plt.show()
...
fig.tight_layout()
plt.savefig(exploratory / 'rolling_betas.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig(exploratory / 'forecasts_r.png', dpi=300)
plt.show()
...
plt.grid(axis='both', linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig(exploratory / 'forecasts_g.png', dpi=300)
plt.show()
...
plt.grid(axis='both', linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig(exploratory / 'forecasts_s.png', dpi=300)
plt.show()
...
plt.savefig(exploratory / "illustrative_debt_dynamics_1.png")
plt.show()
...
'''fig.suptitle("Debt Dynamics w/Static Variables – Level, Slope, Curvature w/Interest Rate Feedback", 
             fontsize=14, weight='bold')'''
fig.tight_layout(rect=[0, 0, 1, 0.97])
plt.savefig(exploratory / "illustrative_debt_dynamics_2.png")
plt.show()
...
# Persist the full-sample β_r and ρ for the simulator (script 03).
pd.DataFrame({
    'parameter': ['beta_r', 'rho'],
    'estimate':  [res_full_fd.params['d_b_lag'], res_full_fd.params['d_r_lag']],
}).to_csv(clean_data / 'term_premium_calibration.csv', index=False)
...
# LaTeX table
from statsmodels.iolib.summary2 import summary_col
# Note: summary_col already emits R-squared (and R-squared Adj.) from each model,
# so we don't add 'R$^2$' to info_dict — that produced a duplicate row.
info_fd = {
...
# Patch in the missing label so \ref{tab:r_estimation_results} resolves,
# and replace bare '<' with '$<$' so the asterisk legend renders without T1 fontenc.
latex_str_fd = latex_str_fd.replace('\\label{}', '\\label{tab:r_estimation_results}')
latex_str_fd = latex_str_fd.replace('p<.', 'p$<$.')
print(latex_str_fd)
with open(tables / 'acm_tp_regressions_first_diff.tex', 'w') as f:
